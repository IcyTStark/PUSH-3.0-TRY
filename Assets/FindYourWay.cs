...
public class FindYourWay : MonoBehaviour
{
    ...
    {
        targetPoint = FindObjectsOfType<TargetPoint>().Where(t => !t.isFilled).OrderBy(t => (t.transform.position - this.gameObject.transform.position).sqrMagnitude).FirstOrDefault();
        return targetPoint;
    }
